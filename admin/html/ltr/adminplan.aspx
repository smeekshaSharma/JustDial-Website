...
<script runat="server">
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        If t1.Text = "" Or d1.Text = "Select Any One" Or d2.Text = "Select Any One" Then
        ...
        conn.Open()
        command = New Data.SqlClient.SqlCommand("select count(*)from plandetail where vt='" & d1.Text & "'and pd='" & d2.Text & "'")
        command.Connection = conn
        count = command.ExecuteScalar
        If (count > 0) Then
            MsgBox("This Plan Already Exist")
           
            conn.Close()
        Else
            sql = "insert into plandetail values('" & d1.Text & "','" & d2.Text & "','" & t1.Text & "')"
            datareader.InsertCommand = New Data.SqlClient.SqlCommand(sql, conn)
            datareader.InsertCommand.ExecuteScalar()
            MsgBox("Your plan has been saved")
            conn.Close()
    ...
    Protected Sub Button2_Click1(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        count = command.ExecuteScalar
        If (count > 0) Then
            command = New Data.SqlClient.SqlCommand("select pd from  plandetail where vt='" & d1.Text & "'and pd='" & d2.Text & "'")
            command.Connection = conn
    ...
    Protected Sub Button3_Click1(ByVal sender As Object, ByVal e As System.EventArgs)
        conn = New Data.SqlClient.SqlConnection("Data Source=.\SQLEXPRESS;AttachDbFilename=|DataDirectory|\humandata.mdf;Integrated Security=True;Connect Timeout=30;User Instance=True")
        conn.Open()
        command = New Data.SqlClient.SqlCommand("delete from plandetail where vt='" & d1.Text & "'and pd='" & d2.Text & "'")
        command.Connection = conn
        count = command.ExecuteScalar
        MsgBox(" Plan Has Not Found")
        d1.Text = ""
        d2.Text = ""
        t1.Text = ""
    ...
    Protected Sub Button4_Click1(ByVal sender As Object, ByVal e As System.EventArgs)
        conn = New Data.SqlClient.SqlConnection("Data Source=.\SQLEXPRESS;AttachDbFilename=|DataDirectory|\humandata.mdf;Integrated Security=True;Connect Timeout=30;User Instance=True")
        conn.Open()
        command = New Data.SqlClient.SqlCommand("update plandetail set pd='" & d2.text & "', amount='" & t1.Text & "' where vt='" & d1.Text & "'and pd='" & d2.Text & "'")
        command.Connection = conn
        count = command.ExecuteScalar
        MsgBox(" Plan Has Been Updated")
        d1.Text = ""
        d2.Text = ""
        t1.Text=""
    End Sub
  ...
  <div id="apDiv4">
    ...
    <div id="apDiv6">
      <table width="99%" height="407" border="0">
        <tr>
          <th width="23%" scope="row"><div align="left" class="style10">Vendor Type</div></th>
          <td width="70%">
        ...
        <tr>
          <th scope="row"><div align="left" class="style10">Payment Duration</div></th>
          <td>
              <asp:DropDownList ID="d2" runat="server" Height="19px" Width="144px">
                  <asp:ListItem>Select Any One</asp:ListItem>
        ...
        <tr>
          <th scope="row"><div align="left" class="style10">Amount</div></th>
          <td>
        ...
        <tr>
          <th scope="row">
              <asp:Button ID="Button1" runat="server" onclick="Button1_Click" Text="Save" />            </th>
          <td>
        ...
        <tr>
          <th scope="row">
              <asp:Button ID="Button3" runat="server" Text="Delete" 
                  onclick="Button3_Click1" />            </th>
          <td>&nbsp;              
              <asp:Button ID="Button4" runat="server" Text="Update" 
                  onclick="Button4_Click1" />            </td>
          <td>&nbsp;</td>
        </tr>
        </table>
    </div>
